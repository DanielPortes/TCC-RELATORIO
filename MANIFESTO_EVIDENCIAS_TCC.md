...
| `figuras/diagrama_neuronio_artificial_proprio.pdf` / `.png` | Elaboracao propria adaptada de Tzanis e Alimissis (2021), gerada por `scripts/gerar_figura_neuronio_artificial.py`; notacao corrigida para `w_1`, `w_2`, `w_d` e `ŷ`. | Capitulo 2, analogia entre neuronio biologico e artificial. |
...
| `figuras/diagrama_seq2seq_attention_proprio.pdf` / `.png` | Elaboracao propria redesenhada como diagrama conceitual de Seq2Seq com atencao para previsao multi-horizonte; fonte em `scripts/gerar_diagramas_modelos.py`. | Capitulo 2, fundamentacao teorica de encoder-decoder com atencao, covariaveis futuras conhecidas e horizonte \(H\). |
