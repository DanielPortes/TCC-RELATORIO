...
O estudo utiliza medições horárias da estação Sapo, pertencente ao recorte CMD (Conceição do Mato Dentro, MG) do conjunto de dados usado neste trabalho. A fonte bruta pertence ao monitoramento contínuo de qualidade do ar disponibilizado pela Fundação Estadual do Meio Ambiente de Minas Gerais \cite{feam2026dadosmonitoramento}. A variável-alvo é a concentração de PM2.5, e o principal poluente auxiliar mantido no contrato final é PM10. Embora a fonte bruta contenha outras variáveis meteorológicas e gases, essas colunas foram removidas do contrato principal porque apresentavam alta esparsidade na estação Sapo e prejudicavam a coerência do conjunto de dados final.
...
\begin{table}[htb]
    ...
    \begin{tabular}{ll}
        ...
        \midrule
        Estação & Sapo / CMD (Conceição do Mato Dentro, MG) \\
...
Foram comparadas cinco variantes neurais e um modelo de referência tabular. A Tabela \ref{tab:modelos_metodologia} resume o papel de cada modelo no desenho experimental.
...
\begin{table}[htb]
    ...
    \begin{tabular}{lll}
        ...
        Seq2Seq atenção com L1 ponderada & Codificador-decodificador com atenção e perda ponderada para regimes altos & Variante final de Seq2Seq \\
        XGBoost com saída múltipla & Árvores com saída multi-horizonte & Modelo de referência tabular forte \\
        \bottomrule
    \end{tabular}%
    }
\end{table}
...
\begin{table}[htb]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{0.19\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.48\textwidth}}
        ...
        Seq2Seq atenção + L1 ponderada & Arquitetura e perda & codificador/decodificador 64; 96; 128; 160; atenção de entrada 64; 96; 128; 160; representação de horizonte 8; 16; 24; 32; pesos moderado/alto e fator de subestimação \\
...
\begin{table}[htb]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.35\textwidth}>{\raggedright\arraybackslash}p{0.35\textwidth}}
        ...
        Seq2Seq atenção + L1 ponderada & codificador 96; decodificador 96; 1 camada; atenção de entrada 96; representação de horizonte 24 & taxa nominal de aprendizado 0{,}007; decaimento de pesos $10^{-6}$; \textit{dropout} 0{,}30; pesos 1{,}20/1{,}10; fator de subestimação 1{,}50 \\
...
A formulação final do Seq2Seq priorizou a variante com atenção de entrada e perda \texttt{weighted\_l1}. Essa perda pondera regimes de concentração e tipos de erro, alterando o objetivo de treinamento para reduzir parte da suavização e melhorar o comportamento em concentrações mais altas. Essa variante foi avaliada no protocolo consolidado com \textit{walk-forward} e múltiplas sementes, enquanto defasagens explícitas e combinações de predições permaneceram como análises auxiliares.
