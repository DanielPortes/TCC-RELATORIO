...
\begin{table}[htb]
    ...
    \begin{tabular}{ll}
        ...
        Imputação & Linear no contrato final, com indicadores de valores originalmente ausentes \\
...
Além da cobertura e da distribuição marginal, o artefato Sapo foi analisado quanto a tendência, persistência e sazonalidade. Essa análise usa apenas pontos originalmente observados nas métricas agregadas, identificados pelos campos \texttt{PM2,5\_\_miss} e \texttt{PM10\_\_miss}. A Figura \ref{fig:serie_temporal_componentes_sapo} resume visualmente a decomposição descritiva do PM2,5: há uma componente lenta de tendência-ciclo, um perfil horário repetitivo e resíduos com picos abruptos.
...
Para PM2,5, o indicador de ausência identifica quais horários têm medição real. Na avaliação final, as métricas são calculadas somente nos pares previsão--observação com medição disponível no teste. Assim, o modelo pode receber uma série regularizada para formar janelas, mas os resultados finais continuam vinculados a medições reais.
...
A comparação com XGBoost exige uma ressalva metodológica. O modelo de referência baseado em árvores usa o mesmo contrato causal de entrada e é avaliado no mesmo bloco final de teste, mas seu treinamento com saída múltipla exige janelas com horizonte completo observado. Nos modelos neurais, pontos sem medição real de PM2,5 não contribuem para a perda de treinamento. Assim, a avaliação de teste é comparável, mas o mecanismo de treino não é perfeitamente simétrico.
