...
A média com múltiplas sementes mantém a LSTM direta como melhor modelo por MAE, mas o novo Seq2Seq atenção passa a ser o segundo colocado com margem pequena: MAE médio 2,8218 contra 2,8055. A LSTM recursiva passa a ter melhor RMSE e \(R^2\) médios, ao custo de viés positivo consistente. Essa diferença confirma que a escolha do vencedor depende da métrica priorizada. Para o objetivo principal do TCC, que usa MAE observado como métrica central de erro médio, a LSTM direta continua sendo a escolha principal; para eventos altos, a análise de cauda favorece o novo Seq2Seq.
...
\begin{table}[htb]
    ...
    \begin{tabular}{lrrrrr}
        \toprule
        \textbf{Variante} & \textbf{MAE} & \textbf{\(\Delta\) MAE} & \textbf{RMSE} & \textbf{\(R^2\)} & \textbf{MBE} \\
        \midrule
...
O resultado reforça uma decisão metodológica importante do TCC. Tratar valores imputados do alvo como observações reais piorou o MAE observado em 0,2197 e reduziu o \(R^2\). Trocar a imputação linear por KNN mantendo a máscara também piorou o resultado. A combinação linear + máscara, portanto, não é apenas uma escolha conveniente de pré-processamento; ela protege a avaliação contra uma fonte concreta de distorção.
